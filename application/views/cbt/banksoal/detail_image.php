...
<div class="content-wrapper bg-white pt-4">
                                            ...
                                            <div class="rTableRow">
                                                ...
                                                <div class="rTableCell align-top">
                                                    ...
                                                    <?php if (isset($jawaban['model']) && $jawaban['model'] == '1') :
                                                        $kolom = json_encode(unserialize($s->opsi_a));
                                                        $baris = json_encode(unserialize($s->opsi_b));
                                                        ?>
                                                    ...
                                                    <?php else :
                                                        $kolom = unserialize($s->opsi_a);
                                                        $baris = unserialize($s->opsi_b);
                                                        ?>
                                                        <table class="table table-bordered">
                                                            <?php
                                                                ...
                                                                if ($nomor === 0) : ?>
                                                                    <tr class="text-center">
                                                                        <?php foreach ($items as $head) :
                                                                            $head = $head == "#" ? "PERNYATAAN" : rawurldecode($kolom[$head]); ?>
                                                                            <th><?= $head ?></th>
                                                                        <?php endforeach; ?>
                                                                    </tr>
                                                                <?php else : ?>
    ...
    <!--
    <button class="btn btn-secondary" onclick="getDataLinked()">export</button>
...
<?= form_close() ?>
<script src="<?= base_url() ?>/assets/plugins/fields-linker/fieldsLinkerImage.js"></script>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $.each($div_lists, function () {
            ...
            var kolom = JSON.parse(JSON.stringify($(this).data('kolom')));
            var baris = JSON.parse(JSON.stringify($(this).data('baris')));
            ...
            var valsA = [];
            var valsB = [];
            $.each(datas.jawaban[0], function (i, v) {
                valsA.push(decodeURIComponent(baris[v]));
            });

            $.each(datas.jawaban[1], function (i, v) {
                valsB.push(decodeURIComponent(kolom[v]));
            });
            ...
            fieldsLink = $('#graph' + noSoal).fieldsLinker("init", inputs);
            fieldsLink.fieldsLinker('disable');
        });
        ...
        setTimeout(function () {
            ...
            fieldsLink.fieldsLinker('enable');
            //fieldsLink.fieldsLinker('manageResize');
            /*
            $(".list-jodohkan").find('p').each(function () {
                $(this).replaceWith($(this).html() + "<br/>");
            });
            $(".list-jodohkan").wrapInner("<p></p>");
             */
            //fieldsLink.fieldsLinker("changeParameters",{"whiteSpace":"nowrap"});
            }, 500);
    });
    ...
    $(document).ready(function () {
        ...
        for (let i = 0; i < 5; i++) {
            listA.push(i+1);
            listB.push(alphabet[i]);
            valsA.push('value ' + (i+1) + '<img src="'+base_url+'assets/app/img/ic_desk.png" class="img-fluid" alt="User Image">');
            valsB.push("value " + alphabet[i]);
        }
        var inputs = {
            "localization": {},
            "options": {
        ...
        linked = $("#original").fieldsLinker("init", inputs);
    });

    function getDataLinked() {
        var results = linked.fieldsLinker("getLinks");
